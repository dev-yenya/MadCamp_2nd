...
import com.example.second_app.PreferenceHelper.set
import com.example.second_app.PreferenceHelper.get


class SharedManager(context: Context) {
    private val prefs: SharedPreferences = PreferenceHelper.defaultPrefs(context)

    fun saveUserInfo(userInfo: UserInformation) {
        prefs["id"] = userInfo.id
        prefs["rating"] = userInfo.rating
        prefs["username"] = userInfo.username
    }

    fun getUserInfo(): UserInformation {
        return UserInformation(
            id = prefs["id", "placeholder"],
            rating = prefs["rating", 0],
